...
PyVista visualizer for the solved skull-implant benchmark.

The script reads x y z_table B C from the solved IK file, reconstructs the
machine geometry and draws the nozzle-tip trace. The optional XYZ path offset
affects only the displayed solved path.

Machine geometry and file paths are read from S00_skull_implant_parameters.py.
"""
...
ANIMATION_PATH = BASE_DIR / "03_skull_implant_visualization.mp4"
...
# Save the animation instead of running interactively.
SAVE_ANIMATION = True
...
# Draw the trace as a tube when required.
VIZ_TRACE_AS_TUBE = False
VIZ_TRACE_RADIUS = 0.25

VIZ_DRAW_TRACE_LIVE = True
VIZ_TRACE_REDRAW_EVERY = 2

VIZ_SHOW_DEBUG_FRAMES = False
VIZ_TRIAD_SCALE = 20.0

# Trace colour mapping.
VIZ_TRACE_GRADIENT = True

VIZ_TRACE_CMAP = "turbo"

# Show the scalar bar for the gradient trace.
VIZ_TRACE_SHOW_SCALAR_BAR = False

# Solid trace colour when the gradient is disabled.
...
# ============================================================
# Visualized path offset
# ============================================================

# Constant offset applied to the solved IK path for visualization only.
# Positive X/Y follow the visualizer axes.
# Positive Z moves the displayed print path upward relative to the bed.
VIZ_PATH_OFFSET_X_MM = -150.0
VIZ_PATH_OFFSET_Y_MM = -150.0
VIZ_PATH_OFFSET_Z_MM = +5.0
...
def main():
    ...
    data_full[:, 0] += VIZ_PATH_OFFSET_X_MM
    data_full[:, 1] += VIZ_PATH_OFFSET_Y_MM
    data_full[:, 2] += VIZ_PATH_OFFSET_Z_MM

    print(f"Loaded {len(data_full)} IK rows from {SOLVED_IK_PATH.name}")
    print(
        f"Path offset: X {VIZ_PATH_OFFSET_X_MM:+.3f}, "
        f"Y {VIZ_PATH_OFFSET_Y_MM:+.3f}, "
        f"Z {VIZ_PATH_OFFSET_Z_MM:+.3f} mm"
    )
