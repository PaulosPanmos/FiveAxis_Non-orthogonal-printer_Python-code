...
Numerical inverse kinematics for the bent-pipe benchmark.
...
This is the numerical alternative to S02b_bent_pipe_ik_solver_hybrid.py.
"""
...
# Positive-C penalty and branch seeding.
...
# Numerical C range used for this benchmark.
IK_C_MIN_RAD = -np.pi
IK_C_MAX_RAD = 0.5 * np.pi
...
def fk_tip_world(q):
    ...
    p_bracket_end = p_b_bracket + R_B @ np.array([0.0, 0.0, -0.5 * P.B_BRACKET_LEN], dtype=float)
...
def solve_bc_for_direction(
    ...
    for it in range(max_iters):
        ...
        # machine C range: roughly -180 deg to +90 deg
...
def solve_cl_to_q5(cl_path=P.CLDATA_PATH, out_path=P.SOLVED_IK_PATH):
    ...
    for k in range(N):
        ...
        if k == 0:
            ...
            cb_ref = None
            cb_sol, ok_bc, _, _ = solve_bc_for_direction(
                d_des_world=d_des_world,
                cb0=cb_seed,
                cb_ref=cb_ref
            )
        else:
            # If desired direction barely changed, keep BC unchanged.
            if d_prev is not None and np.linalg.norm(d_des_world - d_prev) < IK_DIR_HOLD_EPS:
                ...
                # Try both coupled B/C seed directions.
                ...
                # Apply the C limits to both seeds.
        ...
        ok = ok_bc and (dir_err_final < P.IK_TOL_DIR) and (pos_err < P.IK_TOL_POS_MM)
    ...
    print(f"IK complete: {N} rows, {fails} failed")
